...
export async function getDistance(origin, destination) {
  const response = await fetch('https://maps.googleapis.com/maps/api/distancematrix/json?units=metric&origins=' + origin.latitude + ',' + origin.longitude + '&destinations=' + destination[0] + ',' + destination[1] + '&key=' + GOOGLE_API_KEY, {
    method: 'GET',
  })
  const body = await response.json()
...
export async function getRestaurantData(id) {
  ...
  const restaurantData = {
    ...
    photos: body.photos,
    price: body.price,
    hours: body.hours[0].open.map((value) => [value.start, value.end, value.day])};
